...
| # | Pekerjaan | Catatan |
|---|---|---|
| ~~D1~~ | ~~**Perbaiki algoritma `perbaiki_skala_keuangan.py`**~~ | **SELESAI 19 Agu 2026** (`8ecabb81`, jejak #201). Akarnya BUKAN median seluruh riwayat — jangkarnya memang sudah tetangga terdekat; yang salah `len(set(suara))==1` membaca SATU jangkar bicara + satu diam sebagai sepakat, plus penimpaan di tempat yang membuat kesalahan permanen. Sekarang berjangkar pada `_arsip-mentah/` (`dasar_arsip.py`) dan idempoten. Sapuan 949 berkas × 6.574 periode: 3 korban (ZBRA/ARGO/SGER), 35 nilai dikembalikan |
| D12 | **ARGO 2020 & 2021 tersimpan 1000× — belum bisa ditambal** | Temuan dari D1. Tersimpan 80,19 & 78,70 miliar berlabel **USD**, artinya $80 MILIAR untuk Argo Pantes. Nilai benarnya $80,19 jt & $78,70 jt (silang: 2019 arsip $85,03 jt; 2022 arsip IDR 1,13 T ÷ 14.000 = $80,6 jt). Keduanya **tak punya arsip mentah** — 2020/2021 cuma 2 berkas — jadi penambal tak punya dasar dan sengaja diam. Ini satu-satunya lompatan neraca >50× tanpa label yang BERTAMBAH sesudah D1, dan itu memang tujuannya: cacatnya jadi kelihatan, bukan tersembunyi di balik 2019 yang ikut dinaikkan |
| ~~D2~~ | ~~**Kolom C: 10 kuartal diskret, nol jaringan**~~ | **SELESAI 19 Agu 2026** (`b3e2e52b`, jejak #198). 827 dari 949 berkas bertambah interim 2024 TW1 (787 emiten) & TW2 (348); diskret 9.665 → **10.800** periode. Nol jaringan. **Batasnya wajib diingat:** gerbang yfinance TIDAK bisa memvonis 2024 — yfinance cuma menyimpan 2024-12-31 sementara yang masuk 2024-03-31 dan 2024-06-30, jadi irisannya nol dan hasil ujinya nol baris. Itu *tak teruji*, bukan lulus. Penggantinya gerbang kewajaran vs audit tahunan: median Q1 **0,237** (n=665), Q2 **0,243** (n=250) |
...
| ~~D5~~ | ~~`/kartu?kode=XXXX` tak dikenal → halaman kosong~~ | **TERNYATA SUDAH JADI** — diperiksa 19 Agu: `KartuAnalisa.tsx:163` dan `:344` sudah mencetak "Kartu {kode} belum tersedia.". Barisnya lolos dari audit sebelumnya karena tak pernah dicoba, cuma dibaca dari ingatan |
...
| D9 | `potongRentang` jadi kode mati | **Terkonfirmasi 19 Agu**: didefinisikan `grafikEmiten.ts:138`, dipanggil **hanya** oleh 5 tempat di `grafikEmiten.test.ts` dan disebut satu komentar di `kerangkaWaktu.ts:22`. Nol pemanggil produksi. Buang di sapuan kebersihan |
| D10 | Satu baris **`docs/status-panen.md:79`** salah *(bukan `CLAUDE.md` — entri lama menyebut berkas yang keliru)* | "XBRL IDX berhenti di tahun buku 2019" benar untuk DAFTAR laporan, salah untuk ISI-nya |
