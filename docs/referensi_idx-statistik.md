...
| Screener `/screener` | `screener.json`, `pola_screener.json`, `kandidat_deepdive.json` | ⚙️ `app/scripts/bangun-screener.mjs` & `pola-screener.ts` dari `ohlc/` (**jahitan**) + fundamental; **ruas `sektor` sejak 27 Agu 2026 dari `emiten_sektor.json` (IDX-IC resmi, 962/962, keputusan Johan)** — GICS Yahoo di fundamental jadi cadangan, tak dipakai kolom ini; `kandidat_deepdive.json` ❓ produsen belum ditemukan di `scripts/` maupun `app/scripts/` | **ya** | 27 Agu 2026 |
