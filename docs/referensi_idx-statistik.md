...
| Stockbit `exodus.stockbit.com/keystats/<KODE>` | API tidak resmi, token | token; 963/963 | ±94 rasio — dipanen semua | `keystats_stockbit/` | **Kuli Papan** (PBV Band, `kuliPapanData.ts:105`, sejak 23 Agu), **Stock Detail** (rasio tambahan, `rasioTambahanKeystats.ts:134`, sejak 24 Agu), **Berkas Emiten blok F** (94 rasio, `berkasRasio.ts:331`, sejak 29 Agu) | pembaca diverifikasi 30 Agu 2026 |
| Stockbit `exodus.stockbit.com/emitten/<KODE>` (profil & info) | API tidak resmi, token | token; 963/963 | profil: pemegang saham, anak usaha; info: keanggotaan indeks, notasi khusus, UMA | `profil_stockbit/`, `info_stockbit/` | Shareholders (Broker Summary v2) memakai `profil_stockbit/`; `info_stockbit/` dibaca **Neo Papan** (keanggotaan indeks, `neoPapanData.ts:194`, sejak 23 Agu) dan **Berkas Emiten blok G** (notasi khusus & UMA, `BerkasEmiten.tsx:145`, sejak 29 Agu) | pembaca diverifikasi 30 Agu 2026 |
...
| Yahoo Finance `query1.finance.yahoo.com/v8/finance/chart/` | API tidak resmi | bebas, jeda sopan; **`range=max`+`interval=1d` diturunkan diam-diam ke bulanan** → wajib `period1`/`period2` | OHLC harian (**pengisi** `ohlc/` untuk tanggal yang TIDAK dimiliki Stockbit — sejak #161 A (9 Sep 2026) pemanennya MELEWATI tanggal bertanda `sb`, jadi ia tak lagi bisa menimpa sumber utama), IHSG 1990–1999, `^JKSE` cadangan statistik harian, penutupan bulanan seasonality | `ohlc/` (bagian Yahoo), `_arsip-mentah/ohlc-yahoo-sebelum-ganti-volume/`, `ihsg_harian.json` (cadangan), `seasonality/harga_bulanan.json` | Indeks Dunia, Seasonality, dan semua pemakai `ohlc/` | 20–21 Agu 2026 |
...
| Top Broker `/broker` | **utama sejak 7 Sep 2026: `broker/bs_*.json`** (mode hari lewat `brokerHarian.ts:peringkatBroker`, mode rentang lewat `broker_rentang/<preset>.json`); `ds_*` (dataHarian) tinggal **cadangan bertanda** untuk tanggal yang `bs_` -nya belum terpanen | rekap broker resmi bursa (seluruh papan) — cadangan: statistik PDF harian (papan reguler) | tidak (satu sumber; cadangan dipakai utuh, tak dijahit) | 7 Sep 2026 |
...
| Winrate PAPAN `/winrate` *(baru 8 Sep 2026, #91)* | `winrate/<KODE>.json` (956 emiten), `winrate/index.json`; pencarian dari `stock_index` | ⚙️ `scripts/riset/winrate_emiten.py` — mengimpor aturan `scripts/riset/rencana_saham.py` (tidak menyalin) dan membaca `ohlc/` (**jahitan**, deret yang sama dengan Kartu Analisa), `ohlcv_stockbit/` (`foreignbuy`/`foreignsell` untuk arus asing 5 hari), `rencana_saham.json` (blok rencana berjalan); nol jaringan | **ya** (lewat `ohlc/`) | 8 Sep 2026 |
...
Uji silang API profil (`GetCompanyProfiles`, **sumber yang PAPAN pakai**) lawan XBRL laporan emiten sendiri (sheet 1000000, 867 berkas TW3-2025): 3.440/3.468 pasang nilai identik; **8 emiten berbeda nyata** — GIAA (XBRL: Mutual Fund ?! vs API: Airlines), TIRT (Logistics vs Timber), CDIA (Construction Machinery vs Electric Utilities), MGNA (Investment Companies vs Hotels), HERO (Specialty Stores vs Supermarkets — HERO mungkin justru XBRL-nya yang lebih mutakhir, Guardian/IKEA), COCO/LMPI/PBRX (beda tingkat sektor saja). **Keputusan: API profil tetap sumber produksi; XBRL = alat uji silang** (aturan 3b — tidak menjahit/memilih per-emiten tanpa Johan). Kedelapannya dilaporkan ke Johan lewat pengawas. Ejaan kanonis EN dikunci uji `sektorIdx.test.ts` (Properties JAMAK, Logistic TUNGGAL).

| Berkas Emiten `/berkas-emiten` (blok A–G; `/bedah-emiten` ⟶ redirect ke sini) | blok A–D: `ohlcv_stockbit/<KODE>.json` (`candleStockbit.ts:18`), `broker_tahunan/` (`brokerTahunanData.ts:38,55`), `kartu/`, `asing/<KODE>.json`; **blok E**: `bt/`, `rekomendasi/`, `prob/<KODE>.json` + `prob/index.json` (`berkasRekam.ts:125,163,249`); **blok F**: `keystats_stockbit/<KODE>.json` (`berkasRasio.ts:331`) + `fundamental/<KODE>.json` sebagai **cadangan tiga ruas** (`BerkasEmiten.tsx:179`); **blok G**: `info_stockbit/<KODE>.json` (`BerkasEmiten.tsx:145`) | Stockbit chartbit (harga apa adanya, bukan `ohlc/`); ⚙️ `bangun_broker_tahunan.py` dari Stockbit marketdetectors; ⚙️ `kartu_analisa.py`; IDX GetStockSummary (asing); ⚙️ `bangun_prob.py` (mesin `arus-pasar/prob.py`); Stockbit keystats & info; Yahoo lewat `fundamental/` | **ya, tapi hanya blok F dan ditandai per angka** — tiga ruas rasio boleh diisi dari cadangan Yahoo bila keystats kosong (J15). Blok A–D, E, G **tidak** dijahit | 30 Agu 2026 |
| Whales Papan `/whales-papan` | `broker_tahunan/` (`brokerTahunanData.ts`), `ohlcv_stockbit/` (`candleStockbit.ts`), **`/api/live-harga` bar hari berjalan selama bursa buka (`hargaLive.ts`, `gabungBarBerjalan`; tidak ditulis ke arsip, 8 Sep 2026)**, `kartu/ringkas.json` (`kartuRingkas.ts`), `intraday_1h/` (`intradayWhales.ts:6,127`) | ⚙️ `bangun_broker_tahunan.py` dari Stockbit marketdetectors **6 varian**; Stockbit chartbit; ⚙️ kartu; Stockbit intraday 1 jam | tidak — harga & volume dua-duanya dari Stockbit, satu konvensi | 30 Agu 2026 |
| Trader Papan `/trader-papan` | `broker_tahunan/`, `ohlcv_stockbit/` lewat `stockDetailData`/`brokerTahunanData` | ⚙️ `bangun_broker_tahunan.py` (Stockbit marketdetectors); Stockbit chartbit | tidak | 30 Agu 2026 |
| Jago Papan `/jago-papan` | `jago_papan/terbaru.json` (`jagoPapan.ts:14,304`) | ⚙️ turunan pembangun Jago Papan dari arsip broker + OHLCV Stockbit | tidak (turunan satu sumber) | 30 Agu 2026 |
| Kuli Papan `/kuli-papan` (Dev) | `bidoffer/` (`kuliPapanData.ts:52`), `broker_harian/` (`:67`), `keystats_stockbit/` (`:105`), `valuasi_historis.json` (`:106`) | IDX GetStockSummary (bid/offer penutupan); Stockbit marketdetectors; Stockbit keystats; ⚙️ `hitung_valuasi_historis.py` dari XBRL IDX × `ohlc/` × `ListedShares` | **ya** (lewat `valuasi_historis` yang berjangkar `ohlc/`) | 30 Agu 2026 |
| Neo Papan `/neo-papan` (Dev) | `ohlcv_stockbit/` (`neoPapanData.ts:38`), `broker_harian/` (`:72`), `daftar_emiten.json` (`:94`), `kepemilikan/<KODE>.json` (`:188`), `info_stockbit/` (`:194`) | Stockbit chartbit; Stockbit marketdetectors; IDX; KSEI Balancepos; Stockbit info (keanggotaan indeks) | tidak | 30 Agu 2026 |
| Analisa IPO `/ipo` | `ipo.json` (`IpoAnalysis.tsx:40`) | ⚙️ pemanen IPO dari IDX | tidak | 30 Agu 2026 |
...
| J2 | IHSG `ihsg_ohlc_ringkas.json` / `ihsg_harian` = Yahoo 1990–1999 + Stockbit 2000→ | `jahit_ihsg.py` (commit `d217d247`); Yahoo melapor volume dalam **lot**, Stockbit **lembar** (rasio median tepat 100,00) → disamakan | 8.861 bar; volume 0 tinggal 1.261 (semua pra-2000) | agen, dilaporkan ke Johan | **DIPUTUSKAN 5 Sep 2026** bersama J1 — Johan: *"pakai penanda sumber per bar, riwayat lama jangan dipotong"*. Jawabannya "tampilkan dengan penanda", bukan "potong di 2000". `jahit_ihsg.py` kini memakai fungsi penanda yang sama dengan penggabung emiten biasa (`rentang_sumber`, satu rumah, bukan salinan kedua) dan menulis `sumber_bar` ke berkas IHSG. Terukur sesudahnya: 8.870 bar, **1.811 dari cadangan (20,4%)** tersebar di **13 blok** — jadi jahitannya bukan satu potongan pra-2000 seperti dikira, melainkan berselang sampai 2017-12-01. Angka itu sendiri alasan penanda per bar dipilih: penanda "per periode" akan salah untuk 12 blok dari 13. **DIPERTEGAS 9 Sep 2026 (#161 A, keputusan Johan "161 A").** Union-nya tak berubah; yang berubah ATURAN PENULISNYA. Sampai hari itu `panen_ohlc.py` mode harian menarik Yahoo `range=5d` dan menandai tiap tanggal yang ditulisnya `yh` — menimpa tanggal yang sumber utamanya sudah ada. Terukur dari riwayat commit `ohlc/BBCA.json`: 8 Sep 00:27 CI mengubah 1–7 Sep dari `sb` jadi `yh`, diperbaiki tangan 15:17, lalu 8 Sep 23:09 CI mengubahnya lagi untuk 2–8 Sep. Rancangannya mengandaikan penggabung jalan sesudahnya ("Penggabung akan memperhalusnya lagi"), tetapi penggabung TIDAK ada di CI — panen sumber utama hanya lokal — sehingga cadangan menjadi jawaban akhir. Kerusakan angkanya kecil dan disebut apa adanya: harga tutup 2–8 Sep beda **0,00% di seluruh 15 baris** sampel (BBCA/BBRI/BUMI), volume **sama persis di 12 dari 15**, hanya hari TERTUA di jendela yang meleset (Yahoo 4–7% lebih rendah). Yang rusak terutama PELABELANNYA. Sekarang kedua pemanen memakai satu penolong bersama `scripts/penanda_sumber.py`: Yahoo hanya mengisi tanggal yang belum bertanda `sb`, dan penggabung tetap menaikkan Stockbit saat ia datang. **DIPERTEGAS 9 Sep 2026 (#134 D pilihan A, keputusan Johan: `"saya ikuti rekomendasimu dan kerjakan sampai tuntas"`).** Sampai hari itu berkas ini punya DUA penulis yang tak bisa diurutkan: penjahit dijalankan panen lokal, pemanen Yahoo dijalankan CI di awan — dan pemanen menimpa apa pun di tanggal yang ditariknya, termasuk bar yang baru saja ditulis sumber utama. Terukur pada satu jalan: empat bar berganti penanda `sb` menjadi `yh` dan volumenya bergeser 5%, tanpa satu pun galat. Sekarang `panen_ihsg.py` MELEWATI tanggal yang penandanya bukan `yh` (`milik_sumber_utama()`, berswauji): Stockbit utama, Yahoo hanya mengisi tanggal yang tak dimilikinya — praktis sebelum Juli 1997, ditambah hari berjalan yang panen utamanya belum tiba dan akan dinaikkan penjahit. Ikutannya: `ihsg_harian.json` kini ikut membawa `sumber_bar`, dan kartu IHSG di Indeks Dunia merender `CatatanSumberBar` untuk rentang 5 tahun ke atas — sebelumnya bagian cadangan di grafik itu tampil sama meyakinkannya dengan sisanya. |
...
| J6 | `ihsg_harian.json`: cadangan Yahoo `^JKSE` sementara bila PDF IDX belum terbit, ditimpa saat PDF terbit | `panen_ihsg.py` | tercatat di `status-panen.md` 21 Agu | tercatat, Johan tahu | sah sebagai cadangan sementara **asalkan** antarmuka menandai "sementara (Yahoo)" sampai PDF masuk. **Ditutup 9 Sep 2026 bersama J2:** berkasnya membawa `sumber_bar` dan halaman yang menggambar deretnya menyebut bagian cadangan beserta tanggalnya lewat `CatatanSumberBar`; pemanen Yahoo tak lagi boleh menimpa tanggal milik sumber utama |
...
| keystats + profil + info, semua emiten | 2.886 | ≈ 1,8 jam berurutan dengan jeda; keystats dan info HARIAN lewat bat sore (#182 A, keputusan Johan 13 Sep 2026), profil bulanan | ±120 MB | snapshot, jarang berubah |
...
- **Hari berjalan yang belum terbit (diukur 15 Sep 2026):** sebelum IDX menerbitkan ringkasan sebuah tanggal, endpoint menjawab 200 dengan `data: []`, sama persis dengan hari libur. Run CI 15 Sep 00:27 WIB mengarsipkan `_arsip-mentah/asing/2026/20260915.json.gz` berisi 0 baris (64 byte). Bat buka laptop sore itu mengunduh ulang dan masih kosong, sehingga `asing/<KODE>.json` berakhir 14 Sep di commit data sore itu. Pukul 19:50 WIB tanggal yang sama menjawab 963 baris (BBCA ForeignBuy 65.949.600 lembar, Volume 95.608.200, cocok persis dengan volume bar 15 Sep di `ohlc/BBCA.json`). `baris_pasar` (`scripts/panen_asing.py:115`) mengunduh ulang arsip kosong yang berumur 7 hari atau kurang dan hanya menimpanya bila hasil baru berisi, jadi panen berikutnya mengisi tanggal itu sendiri. **Cara membacanya:** `asing/` yang tertinggal satu hari bursa pada malam hari yang sama bukan alarm; baru alarm bila masih tertinggal sesudah panen hari bursa berikutnya. **Tanggal akhir bawaan sejak #199** memakai `akhir_tuntas()`: hari ini hanya sesudah 16:30 menurut jam mesin. Jam itu WIB karena CI `panen-harian-rumah` jalan di runner laptop (`runs-on: self-hosted`). Kalau alur itu dipindah ke runner GitHub (jam UTC), antara 16:30 dan 23:30 WIB fungsi itu meminta tanggal kemarin dan asing tertinggal sehari (catatan pengawas, sidik PGW-0915-AUDIT-7e80310ba).
...
- **Dipakai untuk:** Broker Summary (`brokerHarian.ts:84,104,194`), Broker Summary v2 (Nego, Overview), **Top Broker `/broker` — KEDUA mode sejak 7 Sep 2026**
- **Rotasi sumber 7 Sep 2026 (aturan 3c, keputusan Johan — baris antrean #66 pilihan A):** mode hari Top Broker dulu membaca statistik PDF harian (`ds_*`), mode rentang membaca rekap ini. Satu tabel, dua cakupan papan. Terukur 4 Sep 2026 (juta lembar): XL 12.947 vs 14.159 (1,094×), ZP 4.293 vs 4.551 (1,060×), CC 6.812 vs 6.953 (1,021×), MG 1,022×, YP 1,017×, AK 3.276 vs 3.276 (1,000×) — selisihnya berbeda-beda PER BROKER, jadi ia cakupan papan (reguler vs reguler+negosiasi+tunai), bukan pembulatan; nilai rupiahnya cuma berselisih 1,000–1,008×, konsisten dengan transaksi negosiasi yang menambah banyak lembar berharga kecil. Rekap ini juga lebih lengkap: **765 tanggal vs 155**, dan setiap tanggal `ds_` punya pasangan `bs_` (diperiksa 7 Sep 2026, nol tanggal yatim). **Sumber lama tidak dibuang**: ia cadangan yang dipakai hanya bila `bs_` tanggal itu belum ada, dan halamannya menyatakan di layar bahwa cakupannya papan reguler saja.
...
## Stockbit — `chartbit/<KODE>/price/daily?limit=3` lewat proxy server `/api/live-harga` (bar HARI BERJALAN, 8 September 2026)

- **Jalur:** peramban → `GET /api/live-harga?kode=BNBR` (fungsi serverless Vercel `api/live-harga.js`) → Stockbit chartbit daily `limit=3` dengan token **rantai live** (akun kedua Johan) yang hidup hanya di tabel privat Supabase `live_token` (baris id=1; hanya `service_role`; diputar oleh cron `/api/live-refresh` 00:00 UTC, satu-satunya pelaku refresh). Token tidak pernah ke bundel/klien. Keputusan Johan 28 Agu 2026 (jejak #476) + 8 Sep 2026 antrean #97 A: *"di whales juga bisa dong itu datanya realtime pakai OHLCV nya kan ?"*.
- **Cache:** `Cache-Control: s-maxage=30, stale-while-revalidate=30` (8 Sep: stale diturunkan dari 90); 503 (token belum disemai / 401 / bar kosong / galat) juga disinggah 30 s dan fungsi menyinggah negatif token 30 s. Klien menarik tiap 45 s (Whales) / 60 s (Diary IHSG) hanya selama `jamPasarJakarta().status === 'buka'` (09:00–16:15 hari bursa) — umur terburuk ±2 menit, dan itulah label di halaman.
- **Kamus ruas balasan** (`hargaLive.ts` → `HargaLive`): `kode` · `tanggal` (tanggal bar terbaru chartbit, `YYYY-MM-DD`) · `close` · `prev` (close bar sebelumnya) · `pct` (close vs prev, 2 desimal) · **baru 8 Sep:** `open` · `high` · `low` · `volume` (lembar, sama dengan gudang) · `value` (Rp) · `frequency` (kali). Klien menambah `diambilPada` (epoch ms saat tiba di peramban). Ruas null bila sumber tidak mengirim angka sah.
- **Tidak dikirim, sengaja:** `foreignbuy/foreignsell` bar hari berjalan — diukur 24 Agu 2026 BASI (salinan kemarin) sampai tutup; broker summary hari berjalan juga kosong selama bursa buka (diukur 8 Sep 14:5x, antrean #99).
- **Pemakai:** Beranda Diary Pasar & Indeks (chip IHSG hari berjalan), **Whales Papan** (8 Sep): `gabungBarBerjalan(candle, live, hariIni)` di `candleStockbit.ts` menempelkan satu candle + volume hari berjalan ke deret arsip HANYA bila tanggal live = tanggal Jakarta hari ini dan lebih baru dari bar arsip terakhir, OHLC konsisten; ditempel lewat `series.update()` supaya zoom/geser pengguna tidak direset; sesudah 16:15 bar terakhir ditahan berlabel "PENUTUPAN SEMENTARA · menunggu arsip" sampai `ohlcv_stockbit/<KODE>.json` hasil panen memuat tanggal itu (arsip selalu menang). Bar hari berjalan **tidak pernah ditulis ke arsip** (`buang_bar_hari_berjalan` di panen tetap berlaku).
- **Prasyarat yang masih terbuka (Johan):** tabel `live_token` masih **0 baris** (temuan Papan 8 Sep, #92) — selama itu proxy menjawab 503 dan semua halaman diam-diam memakai arsip harian. Bentuk tabelnya kini punya migrasi di repo: `supabase/migrations/20260908_live_token.sql` (satu baris id=1; RLS aktif SENGAJA tanpa policy, jadi hanya `service_role`).
- **SOP menyemai rantai live (lima langkah, hanya Johan — menyentuh kredensial):**
  1. Buka peramban bersih (jendela penyamaran), login **akun KEDUA** Stockbit. Akun utama tetap bebas dipakai Johan di peramban. **Sejak 8 Sep 2026 (#105/#108) rantainya TUNGGAL:** pemutar satu-satunya cron `/api/live-refresh`, dan pemanen MEMBACA access dari tabel yang sama (`token_segar()` tak pernah memutar). Rumusan lama "dua rantai tak pernah bersentuhan" adalah rancangan 28 Agu yang sudah diganti. Catatan margin: access 24 jam dan cron 24 jam, jadi cadangannya praktis nol — menaikkan ke dua kali sehari butuh paket di atas Hobby, keputusan Johan.
  2. Jalankan `scripts/cek_token_console.js` di konsol peramban itu, tempel dua baris hasilnya ke `app/.env.local` **dengan nama kunci rantai live**: `STOCKBIT_LIVE_TOKEN` dan `STOCKBIT_LIVE_REFRESH_TOKEN` — bukan `STOCKBIT_TOKEN`/`STOCKBIT_REFRESH_TOKEN` yang dipakai rantai panen. Menyamakan namanya berarti satu tempelan salah kamar menukar kedua rantai tanpa satu pun galat.
  3. **Tutup semua tab Stockbit akun kedua.** Refresh token sekali pakai: login peramban sesudah ini memutar pasangannya dan mematikan salinan yang dipegang server.
  4. `python scripts/semai_live_token.py` (butuh `SUPABASE_SERVICE_ROLE_KEY` di lingkungan atau `.env.local` — kunci server tak pernah masuk git). Skrip menolak menulis kalau pemilik tokennya sama dengan rantai panen, dan tak pernah mencetak isi token — hanya tanggal terbit/kedaluwarsa, sidik 8 aksara, dan kode HTTP. `--periksa` memeriksa keadaan tanpa menulis apa pun.
...
- **Akses & batasan:** token akun. Keystats dan info dipanen **harian** lewat bat sore (keputusan Johan 13 Sep 2026, #182 A: rasio yang bergerak mengikuti harga dibaca dari keystats; log sore 11–12 Sep nol penolakan); profil tetap bulanan. Snips ikut `panen-kabar.yml` yang **belum pernah jalan sekalipun** sejak langkahnya ditambahkan 18 Agu.
- **Berkas lokal:** `keystats_stockbit/` (963), `profil_stockbit/` (963), `info_stockbit/` (963), `snips.json` (238, 14 Agu)
- **Dipakai untuk:** `profil_stockbit/` → Shareholders v2 (`brokerProfilKsei.ts:5,136`); `snips.json` → Beranda, Kabar Pasar; `keystats_stockbit/` → **tiga** halaman (Kuli Papan `kuliPapanData.ts:105` · Stock Detail `rasioTambahanKeystats.ts:134` · Berkas Emiten blok F `berkasRasio.ts:331`); `info_stockbit/` → **dua** halaman (Neo Papan `neoPapanData.ts:194` · Berkas Emiten blok G `BerkasEmiten.tsx:145`)
- **Catatan atas klaim lama bahwa keystats & info belum dibaca satu halaman pun.** Kalimat itu benar **saat ditulis** (commit `1baa42477`, 23 Agu **16:28**) dan berhenti benar **empat jam kemudian**: `kuliPapanData.ts` masuk 20:44 dan `neoPapanData.ts` 20:58 hari yang sama. Yang membuatnya bertahan sepekan bukan grep yang salah, melainkan **hasil grep yang dibekukan jadi fakta** — bukti "(grep `app/src` 23 Agu nol)" ikut tercetak di sebelahnya dan justru membuat kalimatnya terbaca sudah terverifikasi. Klaim "belum dipakai" berumur pendek di repo yang aktif; tulis tanggal pemeriksaannya, dan periksa ulang sebelum mengutipnya.
...
| Tersedia (inventaris lengkap) | Diambil? | Alasan / bukti | Keputusan |
|---|---|---|---|
| keystats ±94 rasio (valuasi, profitabilitas, solvabilitas, per-saham, dividen) | ✅ dipanen, ✅ dipakai 3 halaman | Berkas Emiten blok F memajang keseluruhan 94-nya; Stock Detail & Kuli Papan mengambil sebagian | **masih belum diputuskan** — siapa sumber rasio *resmi*. Yang sudah jalan itu pemakaian **berdampingan**, bukan penggantian: yfinance tetap mengisi ruas ringkas `fundamental/`, keystats berdiri sendiri di halamannya |
| profil: pemegang saham, anak usaha | ✅ | Shareholders v2 | Johan (23 Agu) |
| info: keanggotaan indeks, notasi khusus, UMA | ✅ dipanen, ✅ dipakai 2 halaman | Neo Papan (indeks), Berkas Emiten blok G (notasi & UMA — dua bendera paling berbobot karena datang dari otoritas, bukan hitungan kita) | overlap `Remarks` IDX **masih belum diputuskan** |
...
**Pengecualian yang tidak disamakan** (bukan rentang mundur, keputusan yang sama butir 5): tahun awal Musiman, jenis edisi Statistik Berkala, jendela pekan Rotasi, tahun histori Neo Seasonality, jendela 2/3/10/20 hari Broker Stalker, dan Panel Diary pasar di Beranda (sembilan baris performa 1D sampai 5Y ditampilkan sekaligus dari ruas sumbernya, bukan pemilih yang bisa diklik; labelnya dari `LABEL_RENTANG`). **Perluasan 16 Sep 2026** (antrean #210 B dan #211 A atas KERJAKAN pengawas): Top Broker dan Rincian Broker membaca rentang pra-hitung yang kini ditulis dengan definisi yang sama lewat `scripts/jendela_baku.py` (cermin Python `jendelaBaku`); Top Broker punya kesembilan rentang plus Semua, Rincian Broker sembilan rentang tanpa Semua (pivot seluruh arsip terlalu berat tiap sore). Preset panjang Broker Stalker (3 Bulan, 6 Bulan, YTD, 1 Tahun, 2 Tahun, Semua) dihitung J20 lewat `batasKalenderBaku`; jendela 1-20 hari tetap. Definisi lama rollup Top Broker ternyata sudah setara, jadi angkanya tidak bergeser; 1 Tahun Stalker bergeser dari 252 hari bursa ke 239 hari.
...
Stock Detail = **Yahoo + IDX**, dengan tambahan Stockbit sejak 24 Agu (`rasioTambahanKeystats.ts`). KSEI tidak dipakai di halaman ini. Isi tiap berkas Stockbit:

| Berkas Stockbit | Isi | Rentang |
|---|---|---|
...
| `profil_stockbit/<KODE>.json` (963) — Shareholders v2 | `shareholder` (13 ruas: percentage name value type nationality scripless scrip classification…), `subsidiary`, `key_executive`, `shareholder_one_percent`, `beneficiary`, `listing_information`, `history`, `background` | 23 Agu (dipakai hanya Shareholders v2) |
| `info_stockbit/<KODE>.json` (964) — Neo Papan, Berkas Emiten blok G | `indexes` (keanggotaan indeks), `notation`, `uma`, `corp_action`, `sector/sub_sector/industry`, `trading_limit_info`, `margin_info`, `day_trade_info` | 23 Agu |
...
**Arsip hari-H bisa berisi 5 varian, bukan 6 (diukur pengawas 15 Sep 2026 atas 962 folder):** 14 Sep lengkap 962 emiten × 6 varian; 15 Sep 830 lengkap dan 132 emiten hanya 5 varian, semuanya tanpa REGULER dan bervolume 0 hari itu. Pola yang sama muncul 8 dan 9 Sep, yaitu hari yang dipanen pada hari itu juga, sedangkan 10, 11, dan 14 Sep yang dipanen ulang keesokan harinya lengkap 962. Sebabnya `scripts/panen_broker_harian.py` sekitar baris 670-681: REGULER kosong sengaja tidak disimpan karena ia wasit "sumber sudah siap", sedangkan varian lain yang kosong pada emiten tanpa volume tetap disimpan. Ini bukan data hilang. Pemeriksa kelengkapan yang menghitung "6 varian per emiten" wajib mengecualikan emiten bervolume 0, atau ia menandai sekitar 130 emiten sepi sebagai bolong. `scripts/tgl_broker_lubang.py` (#194) aman karena hanya menilai BBCA.
...
### Kamus ruas `winrate/<KODE>.json` dan `winrate/index.json` (8 September 2026, #91)

Dibangun `scripts/riset/winrate_emiten.py` (nol jaringan, ±40 detik untuk 956 emiten) sesudah `rencana_saham.py` di PanenSore dan BukaLaptop; gerbang `cek_kesegaran.py` membaca ruas `akhir` di `index.json`. Dipakai satu halaman: **Winrate PAPAN** `/winrate`. Bukti uji: `winrate_emiten.py --uji` — 45 pembanding data nyata (BNBR, ARCI, BBCA × 6 horizon × 2 jendela) identik dengan `telusuri()` produksi, dan h5/h10/h20 pada 120 sinyal identik dengan `jejak` di `rencana_saham.json` (BNBR h10: 67,0 / 59,2 / −0,250).
...
| `saringan.h20`, `saringan.h60` | 13 kondisi di hari sinyal (jendela 500) → ringkasan bentuk yang sama dengan `horizon`: RSI14 Wilder (<30 · 30–70 · >70), posisi vs MA20·50·200 sederhana (di atas semua · campuran · di bawah semua), volume vs rata 20 hari (>1,5× · normal · <0,5×), arus asing bersih 5 hari dari `ohlcv_stockbit` (net beli · net jual/nol), rentang harian vs median jendela (atas · bawah). Kondisi yang tak bisa dihitung (mis. gudang asing tak ada) tak muncul, bukan dianggap nol | kartu "Saringan teknikal" |
| `barBeku.<tahun>` | `beku` (tinggi = rendah DAN tutup = tutup kemarin), `bar`, `pct` — BNBR 2013–2017 100% beku | kartu "Bar beku per tahun" |
...
### Jejak Rekomendasi (B45, 28 Agu 2026) — `rekomendasi/<tgl>.json`, ⚙️ `scripts/riset/rekap_preset.py`

Satu berkas per tanggal, TIDAK diedit lagi setelah ditulis (kejujuran backtest — koreksi = berkas terpisah).
Bentuk: `{tanggal, dibangun, backtest, presets:[{preset, saham:[...]}]}`. `preset` = id dari `PRESET`
(`presetScreener.ts`: `scalping`, `swing`, `whale-tiket`, `whale-akdis`, `whale-asing`) — ambang & kriteria
DIPORT manual ke Python (`PRESET_DEFS`), jangan disunting satu sisi saja. Maks 20 saham skor tertinggi per
preset per tanggal (bukan seluruh populasi lolos — itu tugas tab Preset, bukan arsip win-rate).

| Ruas `saham[]` | Arti | Data yang dihasilkan |
|---|---|---|
| `close` | harga penutupan hari rekomendasi | dasar semua definisi menang |
| `entry` | `[low hari itu, close]` | rentang masuk yang dipakai edukasi, BUKAN sinyal beli |
| `tp1` `tp2` | target 1×/2× ATR14 dari `close`, dibulatkan ke fraksi harga BEI | target TP/SL H+5 |
| `sl` | `min(low 5 hari, close − 1,5×ATR14)`, dibulatkan ke fraksi harga BEI (arah bawah) | batas rugi TP/SL H+5 |
| `skor` | `lolos ÷ terukur` kriteria preset hari itu (0..1) | urutan top-20 |
| `ringkas.freq` `ringkas.ukuran_order` `ringkas.fd` `ringkas.label_accdist` | disalin apa adanya dari `kartu/arsip/<tgl>.json` | konteks tampilan, bukan dihitung ulang |
| `ringkas.bandar_top1_kode` `ringkas.bandar_top1_avg` | **selalu `null`** — Tugas A #7 spek (net buyer nilai terbesar per broker) belum dibangun di `kartu_analisa.py`; sengaja `null`, bukan ditebak dari `broker_tiket_maks_kode` (konsep beda: itu broker ber-tiket RATA-RATA terbesar) | — |
...
**Push data tidak membangun Vercel, terbukti di produksi 15 Sep 2026 (#192, diukur pengawas):** `scripts/vercel_lewati_build.sh` membandingkan HEAD dengan deployment sukses terakhir di luar `data-idx/`, `arus-pasar/`, dan `docs/`. Klon Vercel hanya 10 commit, jadi SHA pembanding yang lebih tua diambil lewat fetch. Bukti positif: `6ed6a3da1` 20 commit sesudah build terakhir `038653ca4` tetap dibangun karena `scripts/` dan `.github/` berubah (sukses 13:22:47Z). Bukti negatif: commit data `d33c2c71d` (9.611 berkas) dan `c447ec294` tidak punya deployment Production. Satu deployment GitHub Pages untuk `75dceff51` berstatus error pukul 11:24Z dan build Pages berikutnya sukses; kejadian sekali, bukan cacat.
...
- 16 September 2026 — **J20 diperluas** ke rentang pra-hitung Top Broker dan Rincian Broker serta preset panjang Broker Stalker (#210, #211); skrip kalibrasi ambang narasi ikut membaca `ohlc/IHSG.json` (J19, #208).
