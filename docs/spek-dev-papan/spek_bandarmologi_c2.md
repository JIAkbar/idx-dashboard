...
> **⚠️ REVISI LABEL KUADRAN §B.1 — 27 Agu 2026 (sapuan pengawas `spek_label_mengklaim.md` #2).** Label yang spek ini perintahkan ("Akumulasi Cerdas / Beli Agresif / Jual Panik / Distribusi") DIBATALKAN — mengklaim kepintaran & emosi dari data yang cuma tahu beli/jual × posisi vs VWAP. Label yang berlaku: **Beli/Jual di Bawah/Atas VWAP** (menyebut sumbunya); tafsir pindah ke keterangan legenda. Pola yang sama membatalkan vonis arah CPR di panel Analitik ("Bullish kuat" → deskripsi struktural pergeseran rentang CPR) — klaim arah menunggu BT Papan + BadgeRapor.
