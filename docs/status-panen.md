...
| **Broker summary — PER EMITEN** | *(halaman `/broker-summary-v2`)* | **Stockbit `marketdetectors`**, **12 varian** = 3 papan (REGULER/NEGO/TUNAI) × 2 tipe investor (ALL/FOREIGN) × 2 transaksi (GROSS/NET). `DOMESTIC` TIDAK dipanen — terbukti = ALL − FOREIGN, cocok persis | **22 emiten** per 23 Agu 2026 siang (145.834 berkas), gelombang 1 menargetkan 300 teratas urut likuiditas | 145.834 | ⚙️ hari berjalan ikut `panen-harian-rumah.yml` — **masih 1 varian (reguler saja)**, bukan 3 seperti tertulis sebelumnya: langkah 3d memanggil `panen_broker_harian.py` tanpa `--varian`, dan bawaannya `reguler`. Menaikkannya ke 12 menambah beban harian dari 16 menit jadi 3,2 jam pada jeda 1 detik — **belum diputuskan**; riwayat ❌ manual | `backfill_broker_massal.py --paralel 12` — **"Panen Lagi"** |
