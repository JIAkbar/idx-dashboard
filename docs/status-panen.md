...
| **Broker tahunan (olahan)** | Trader Papan, Neo Inventory/Compare/Stalker (jalur tahunan) | ⚙️ `bangun_broker_tahunan.py` dari `_arsip-mentah/broker-harian/` — nol jaringan | **2016 → 2026 per 27 Agu 2026** (keputusan Johan "gpp sampai 2016"; sebelumnya 2020→). 491 emiten punya 2016; BBCA 2016 = 246 hari, cocok persis arsip mentah (sampel PD 229.605 lot / Rp 59,49 M); index per emiten dibaca dari DISK (run parsial tak menghapus tahun lain); 61 EINVAL indeks Windows (kunci berkas transien) diperbaiki 61/61. **8 Sep 2026: seluruh 2026 dibangun ulang berlarik 8 kolom** (#81) — lima kolom lama di tempatnya, ditambah frekuensi beli, frekuensi jual, jenis broker (A/L/P); 881 emiten, 134.274 hari, 3.615.572 baris, nol berkas rusak, 10 hari acak cocok persis arsip mentah. Tahun 2016-2025 masih 5 kolom dan itu dibaca pembacanya sebagai "belum tahu", bukan nol | 10.400+ berkas, 2,1 GB | ❌ manual, nol jaringan | `C:/Python314/python.exe scripts/bangun_broker_tahunan.py` (dukung `--tahun 2016,2017` subset · `--paralel 8` · `--lanjut` resume) — ikut **"Panen Lagi"** setelah panen broker harian |
...
| **Keystats Stockbit (94 rasio)** | Stock Detail: lima rasio utama sejak 8 Sep 2026 (P/BV, P/S, Asset Turnover, Div Yield, Altman Z), panel Rasio Perbankan/Multifinance, Peringkat Antar Emiten IDX · Tanya PAPAN dan Kartu Analisa (P/BV) | Stockbit key statistics, 12 kelompok rasio | **2026-09-12** (962 dari 963 berkas; dibaca dari `dipanen_pada` di dalam berkas, 13 Sep 2026) | 963 | ⚙️ bat sore HARIAN (#182 A, keputusan Johan 13 Sep 2026) | `JALANKAN_PANEN_SORE.bat` · manual **"Panen Lagi"** |
| **Profil Stockbit** | Broker Summary v2 → tab Shareholders (pemegang saham/anak usaha/direksi, agen lain); Stock Detail sejak 24 Agu 2026 (alamat, latar belakang, sekretaris, ringkasan IPO) | Stockbit company profile | **2026-08-23** (962 dari 963 berkas, termuda 2026-08-29; dibaca dari `dipanen_pada` di dalam berkas, 13 Sep 2026) | 963 | ⚙️ Buka Laptop, sebulan sekali (`gerbang_bulanan.py profil_stockbit 28`, #170 B) | **"Panen Lagi"** |
...
| **Winrate PAPAN** *(baru 8 Sep 2026, #91)* | Winrate PAPAN (`/winrate`) | turunan lokal: arsip OHLC × gudang Stockbit (arus asing) × `rencana_saham.json` — **nol jaringan** | **2026-09-07** (ruas `akhir` DI DALAM `winrate/index.json` = modus bar terakhir 956 emiten; dibangun 8 Sep 2026 13:18) | 957 berkas (956 emiten + `index.json`), 12 MB | ✅ otomatis — PanenSore + BukaLaptop, sesudah `rencana_saham.py`, ikut `git add`/`git commit` keduanya | `scripts/riset/winrate_emiten.py` (±40 detik); gerbang `cek_kesegaran.py` baris "Winrate PAPAN" |
| **Rapor Uji** *(baru 23 Sep 2026, #221)* | Rapor Uji (`/rapor-uji`) | turunan lokal: arsip OHLC × pilihan redaksi (edisi + Deep Dive) × log sinyal OBV/RBS yang dicatat tiap hari — **nol jaringan** | log dimulai 23 Sep 2026 (ruas `mulai` DI DALAM `rapor_uji/log_sinyal.json`; `data_per` di `rapor_uji.json`) | 2 berkas (`rapor_uji.json`, `rapor_uji/log_sinyal.json`) | ✅ otomatis — PanenSore + BukaLaptop, sesudah `winrate_emiten.py`, ikut `git add`/`git commit` keduanya | `scripts/riset/rapor_uji.py --catat` lalu `--nilai`. Log APPEND-ONLY: jangan pernah dibangun ulang atau disunting, karena nilainya sebagai uji ke depan hilang begitu baris lama berubah |
| **Ringkasan broker harian** *(baru 23 Sep 2026, #231)* | PAPAN Baru: Peta Pasar Hidup (`/baru/pasar`), Kartu Pagi (`/baru/pagi`), Emiten sebagai cerita (`/baru/emiten/:kode`) | turunan lokal dari `broker_tahunan/` (reguler, semua investor, gross) × kelompok broker (`kelompokBroker.ts`) × sektor (`emiten_sektor.json`) — **nol jaringan** | ruas `tanggal` DI DALAM `peta_arus.json` | `peta_arus.json` + `broker_puncak/<KODE>.json` (10 hari per emiten) | ✅ otomatis — PanenSore + BukaLaptop, sesudah `bangun_broker_tahunan.py`, ikut `git add`/`git commit` keduanya | `scripts/riset/ringkas_broker_harian.py --tulis`. Kelompok = identitas broker perantara, bukan jenis investor |
...
| `keystats_stockbit/` | Stock Detail (lima rasio utama), Tanya PAPAN, Kartu Analisa, Kuli Papan, rasio tambahan | Stockbit (token) | **2026-09-12** (962 dari 963; dibaca dari `dipanen_pada` di dalam berkas, 13 Sep 2026) | 963 | ⚙️ bat sore harian (#182 A); Buka Laptop hanya saat profil jatuh tempo (guard arsip-hari-ini) | `python scripts/panen_keystats_stockbit.py --semua --jeda 0.4` |
| `info_stockbit/` | Neo Papan (profil) | Stockbit (token) | **2026-09-12** (962 dari 963; dibaca dari `dipanen_pada` di dalam berkas, 13 Sep 2026) | 963 | ⚙️ bat sore harian (#182 A); Buka Laptop hanya saat profil jatuh tempo (guard arsip-hari-ini) | `python scripts/panen_info_stockbit.py --semua --jeda 0.4` |
...
| `bt/rbs-stat-<D\|W\|M>.json` + `bt/gap-stat-<D\|W\|M>.json` + `rbs_kandidat.json` | Grafik Emiten (baris statistik di bawah tombol Pola RBS) · pemasok ide Deep Dive | Turunan MURNI dari `ohlc/` (nol jaringan) lewat mesin RBS yang SAMA dengan yang menggambar garisnya di chart (`scripts/riset/rbs_mesin.py` = `app/src/lib/dasbor/polaRbs.ts`) | **2026-09-07** (perdana): RBS D 21.466 breakout / 956 emiten; Gap D 66.998 zona / 961 emiten (definisi RENTANG, bukan open) · W 2.980 / 951 · M 293 / 735; 86 kandidat sah 30 hari terakhir (BBCA 6.588 sah 3 Sep di antaranya) | 4 berkas `data-idx/json/bt/rbs-stat-*.json` + `data-idx/json/rbs_kandidat.json` | ⚙️ otomatis — kedua bat panen (`[E] Turunan`) + CI rumah langkah `4c` | `python scripts/riset/rbs_statistik.py --kerangka D W M --tulis` + `node app/scripts/gap-statistik.ts --tf=<D\|W\|M> --tulis` — ikut **"Panen Lagi"**. Kerangka yang berkasnya TAK ADA membuat chart DIAM soal statistik, bukan memajang angka basi |
| `bt/index.json` (run `pivot_cpr.*` / `rr_setup.*`) + `bt/pivot-cpr-<D\|W\|M>.json` | Grafik Emiten — badge rapor di panel Analitik (Pivot/CPR & R:R) | Turunan MURNI dari `ohlc/` (nol jaringan) lewat mesin klasifikasi yang SAMA dengan yang menggambar pita CPR di chart (`app/src/lib/dasbor/chartAnalitik.ts`) | **2026-09-07** (perdana): 21 run — 6 kelas berarah × 3 kerangka + setup R:R × 3 kerangka. D 962 emiten 2000-03-30–2026-09-04 (laju dasar long 46,29% dari 1.709.747 bar) · W 46,87% · M 47,60% | 1 index + 3 berkas rinci | ⚙️ otomatis — kedua bat panen (`[E] Turunan`), tepat sesudah statistik Gap | `node app/scripts/bt-pivot-cpr.ts --tulis` (D+W+M sekali jalan, ~15 detik) — ikut **"Panen Lagi"**. Kerangka tanpa run membuat badge TAK TAMPIL, bukan memajang angka kerangka lain |
| `broker_rentang/<w1\|w2\|b1\|b3\|b6\|sejakJan\|y1\|y2\|semua>.json` (sejak 16 Sep 2026, definisi J20; `h5`/`ytd` lama tak lagi dibaca) | Top Broker (`/broker`) — pil rentang di samping tanggal | Turunan MURNI dari `data-idx/json/broker/bs_*.json` (rekap broker harian resmi, 88 broker lengkap) — sumber yang SAMA dengan mode Hari Ini, bukan jahitan vendor lain | **2026-09-07** (perdana): 765 hari bursa tersedia 2023-06-15 s.d. 2026-09-04; YTD 158 hari | 5 berkas `data-idx/json/broker_rentang/*.json` | ⚙️ otomatis — kedua bat panen (`[E] Turunan`) + CI rumah langkah `4d` | `python scripts/bangun_broker_rentang.py --tulis` — ikut **"Panen Lagi"**. Preset tanpa berkas membuat halaman berkata rollup-nya belum ada, bukan memajang tabel kosong |
| `broker_pivot/<KODE_BROKER>.json` | Rincian Broker (`/broker/<kode>`) — akumulasi & distribusi per emiten | Turunan MURNI dari arsip transaksi broker per emiten (`broker_tahunan/`, varian reguler) — arah bacanya dibalik, bukan sumber baru. **Daftar harian per emiten dipotong 50 broker teratas tiap sisi oleh sumbernya**, dan halaman pemakainya wajib menyebut itu | **2026-09-07** (perdana): akhir 2026-09-04; 3 periode (5 hari bursa, 30 hari, 91 hari); sejak 16 Sep 2026 sembilan kunci baku J20 (1 Hari s.d. 2 Tahun), membaca gudang 2024-2026 | 90 berkas `data-idx/json/broker_pivot/*.json` (89 broker + index), 1,8 MB | ⚙️ otomatis — kedua bat panen (`[E] Turunan`) + CI rumah langkah `4e` | `python scripts/bangun_broker_pivot.py --tulis` — ikut **"Panen Lagi"**. Broker tanpa berkas membuat halaman berkata rinciannya belum ada, bukan memajang tabel kosong |
