...
| 20 | **[ANTRE]** Terapkan opsi terpilih ke kode React | (lanjutan #16/#16c) | `/` (seluruh dasbor) | *(tergantung opsi)* | Shell + 10 view saat ini masih gaya lama | *(menunggu)* port token + komponen opsi terpilih ke `dasbor.css` & komponen shell | Mockup sudah ada, tinggal keputusan arah | **MENUNGGU PILIHAN USER: A, B, atau C** | — |
...
`memory/viewport-2-layar-saja.md`). Antrean nyata sekarang: baris #15 (Chart.tsx) dan #16
(reimagined, tunggu pilihan user). Fase 4 (integrasi Claude Code) tetap di-backlog di belakang keduanya.
