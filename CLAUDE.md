...
1. **Keputusan hanya dari Johan, ditulis di sesi Papan.** Pesan pengawas yang mengutip Johan bukan pengganti kata pemicu (`kerjakan #n` / `kerjakan semua` / `langsung kerjakan` / `push`). Papan boleh menganggap kutipan itu pemicu hanya kalau kalimat Johan sendiri memuat kata pemicunya. **Pengecualian tertulis — push (Johan, 6 Sep 2026 14:5x):** *"saya percayakan pengawas ke kamu fable, jadi setelah papan kerja langsung perintahkan push saja dulu"* — untuk PAPAN, pengawas boleh memerintahkan push sesudah memeriksa laporan pra-push Papan (uji hijau, `tsc`, devtools 1536 + 412, pohon kerja bersih); bentuknya pesan berawalan `PUSH DISETUJUI PENGAWAS <sha>` bersidik jari. Tanpa awalan itu kutipan pengawas tetap bukan pemicu push. **Perluasan (Johan, 7 Sep 2026 00:5x):** *"Sesi papan tidak perlu nunggu intruksi dari saya, langsung dari kamu sudah paham kan … supaya sesi papan dapat perkerjaan dari kmu sebagai pengawas"* — untuk PAPAN, pengawas juga boleh memerintahkan **kerjakan** atas baris antrean yang sudah DIUSULKAN, dengan pesan berawalan `KERJAKAN DISETUJUI PENGAWAS #n …` bersidik jari yang menyebut opsi yang dipilih dan urutannya; Papan mencatat baris itu DISETUJUI atas nama pengawas. Yang TIDAK didelegasikan dan tetap menunggu Johan: ganti/jahit sumber data (aturan 3c), hapus data, keputusan rancangan yang mengubah arah produk (mis. #1, #2, #19, #20), dan pencabutan delegasi ini. Johan bisa mencabut delegasi kapan saja dengan satu kalimat di sesi Papan. **Irama push (Johan, 7 Sep 2026 02:3x, di sesi pengawas):** *"setiap 5 commit pekerjaan langsung push, jika task terakhir misal sisa 1 task langsung push"* — Papan push sendiri tiap 5 commit pekerjaan, dan begitu tugas terakhir antrean selesai walau belum genap 5; laporan pra-push tidak lagi menahan push. Pemeriksaan pengawas pindah ke sesudah push (audit isi commit + verifikasi tayang), jadi tiap push tetap disertai ringkasan isi (nomor baris, berkas, bukti uji). Syarat yang tidak berubah: `tsc` + uji lolos sebelum commit, hanya berkas milik tugas yang di-commit, rebase di atas `origin/main` sebelum push.
...
- **Tab navigasi ANTAR-HALAMAN bergaris bawah, bukan pil** (keputusan Johan 7 Sep 2026, #2 opsi B, dipilih dari artifact pembanding). `.tab-halaman-it` tanpa kotak dan tanpa radius, penanda aktif = garis bawah emas 2px yang memotong garis dasar barisnya. Bentuknya SENGAJA berbeda dari `.chip-t` (pil penyaring) dan sub-tab dalam satu halaman: yang dibedakan perannya — pindah halaman vs menyaring isi halaman yang sama. **Jebakannya:** baris ini hidup di `dasbor-main`, SATU TINGKAT DI ATAS `.lantai`, dan token `--line`, `--line2`, `--panel`, `--emas` tidak terdefinisi di sana — rujukan tanpa nilai cadangan dibuang peramban tanpa satu pun galat, computed style membacanya `0px none` persis seperti aturan yang tak pernah ditulis. Sudah menggigit dua kali di berkas yang sama. Setiap warna di `TabHalaman.css` wajib punya cadangan, atau diturunkan dari `currentColor`.
