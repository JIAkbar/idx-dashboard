...
- **Halaman baru WAJIB sistem tata C+A** (keputusan Johan 28 Agu, artifact Re-Layout + "kerjakan semua page seperti whales"): baris kendali = `.bilah-kendali` dengan `.grup-k` berlabel `.grup-lbl` (aksi ke `.grup-kanan`), header = `.vhead` h1 + `span.sub` + `<CatatanCakupan inline />` SEBARIS, panel samping (bila ada) = `.tata-2` kontekstual yang kolaps saat kosong. Kelas & token (--j1/--j2/--j3) di blok "SISTEM TATA C+A" `lantai.css`; percontohan WhalesPapan.tsx. 21 halaman sudah memakainya — halaman baru yang menata sendiri = regresi konsistensi.
