...
def perbarui_ringkas(lama: dict | None, kode: str, tanggal: str, baris: list[list],
                     ringkas: dict, jendela: int = JENDELA_HARI) -> dict:
    """Sisipkan satu hari ke berkas ringkas per emiten, potong ke `jendela`."""
...
def jalankan(a) -> int:
    ...
    for i, kode in enumerate(kode_semua, 1):
        ark = ARSIP / kode / f"{tanggal}.json"
        if ark.exists() and not a.ulang:
            n_lewat += 1
            mentah = baca(ark)
        else:
            st, isi = ambil(token, kode, tanggal)
            if st == 401:
        ...
        if not baris:
            # Nol dari API hanya sah kalau IDX juga menyatakan tak ada volume.
            # Kalau IDX punya volume, datanya belum siap — JANGAN diarsipkan,
            # supaya jalan ulang mencobanya lagi.
            n_kosong += 1
            if vol:
